...
**Subsequent study:** the separately hash-frozen [MOBIO/FEI follow-up](../scheme_followup_2026-09-18/README.md) completed 216 endpoints across two new identity assignments and three model seeds in 859.63 seconds. Its eight primary pool-4 contrasts have positive crossed intervals and Holm p = 0.004; twelve native controls have Holm p = 0.006. These results supplement, not replace, the original pilot tables below.
...
The [per-seed matrix](../multiexposure_run_matrix.csv) contains 849 rows from 49 locally available multi-exposure artifacts, including these 48 pilots and 216 follow-up endpoints but no SCface rows. All previous 633 rows were retained unchanged. SCface compact exports are tracked separately; its detailed artifacts are unavailable on this host. The [coverage audit](../scheme_followup_2026-09-18/coverage_audit.csv) also identifies two legacy-schema MOBIO sources outside the inventory. Five non-multi-exposure artifacts were excluded from the original inventory. This is not proof that every historical study has been recovered. `config_sha256` in that matrix hashes the saved YAML bytes; the pilot table hashes canonical configuration serialization, so the two hashes are not interchangeable. Historical plots still use their tracked source-separated summaries; migration to one fully reconciled matrix remains pending. Missing stage annotations default to exploratory, not confirmation.

The original pilot paired table's `seed` column records bootstrap seed 91223, not trained-model seed 419. The follow-up exporter separates `seed` and `bootstrap_seed`; historical numerical results are unchanged.

The [dated pilot review](../../docs/review/scheme_pilot_review_2026-09-12.md) remains historical. See the [follow-up scope](../scheme_followup_2026-09-18/README.md#remaining-scope) for completed controls and remaining work; the later one-hour authorization did not cover the full cross-dataset roadmap.