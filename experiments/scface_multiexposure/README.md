...
Archive checksum and acquisition steps are retained in the [SCface setup record](../../docs/setup/SCFACE_LOCAL_DATA.md). The [849-row local inventory](../multiexposure_run_matrix.csv) covers 49 artifacts but excludes SCface because its detailed artifacts are unavailable on this host; do not infer run-level coverage from aggregate figures. Separate [IoM-GRP/PolyProtect pilots](../scface_scheme_extension_pilot/README.md) use one model seed. The new [216-endpoint follow-up](../scheme_followup_2026-09-18/README.md) covers MOBIO/FEI only: SCface embeddings were unavailable locally, so no new SCface training or native permutation controls were run. Existing SCface aggregates remain in the nine-page report and 15-figure appendix.
