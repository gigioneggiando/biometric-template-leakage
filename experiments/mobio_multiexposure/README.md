...
This supports chance-compatible learned linkage across a linear sign-projection scheme and a nonlinear random-network scheme; it does not establish statistical equivalence or privacy. MLP-Hash is labelled paper-specified, not source-exact, because the authors' GitLab source was unavailable and the paper's row-orthonormal instruction is dimensionally impossible at its narrowing output layer.
...
The pattern is multiplicity amplification gated by transform diversity: selected pools have small single-record means and `34-54%` ten-record linkage, but this is not universal (dense pool 3 already has `27.50%` single-record linkage). Fresh-key means differ between exposure levels without establishing amplification or equivalence. The boundary is protocol-specific and noisy near pools 6-9 (pool 6 scored below pool 7 with a seed SD of `18.2` points), not a universal threshold.
...
Partition 3 collapses at pool 5 while partitions A and 2 retain a pool-7 effect. Tested pools 3/4 pass the interval criterion in all three partitions; pools 5-7 are partition-dependent, and tested pools 8/9 fail. Failure does not establish absence of leakage. Haar sign-corrected BioHash gives `74.58 / 48.06 / 2.64%` for pools 1/5/fresh, a qualitatively similar pattern, not verified equivalence to the default implementation. MLP-Hash pools 3/4 give `54.31 / 37.78%`; pool 5 fails the strict interval criterion despite a positive mean.
