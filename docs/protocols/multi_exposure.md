...
## Same-image and correlated-key controls (2026-09-05)

Fixed before inspection. Both controls use sign-corrected Haar BioHash, exposures 1 and 10, DeepSets, eight sets per identity, three model seeds, held-out galleries, and new identity partitions.

1. Same-image/different-key: paired configs use split/key/set seeds `90803/90821/90833` and model seeds `439/443/449`. The baseline uses the usual nested different-image sets. In the control, each set chooses the same first source image as its paired one-record baseline and protects that identical normalized embedding under 1 or 10 nested fresh, split-disjoint keys. Different repeats select different source images. Endpoint: 10-record top-1 and AUROC for same-image versus different-image fresh-key sets. Purpose: separate within-person image/session variation from key randomization; both are expected to remain near chance under the theorem.
2. Correlated keys: split/key/set seeds `90839/90847/90863`, model seeds `457/461/463`. Each record retains a unique split-scoped private key, while its sign-corrected 128-dimensional BioHash projection shares an exact prefix of `0/32/64/96/128` projection columns system-wide (`0/25/50/75/100%`). Private columns are orthogonalized against the shared prefix. This is a controlled partial-projection-reuse model, not a claim about a standard key derivation implementation. Endpoint: one- and ten-record top-1 as a function of shared fraction; no monotonicity or crossing point is assumed in advance.

After inspecting the coarse sweep, a fine follow-up was fixed on a new identity partition before inspecting any follow-up result. The coarse 25% condition produced `9.44%` mean 10-record top-1 versus `3.33%` at one record, but only one of three clustered lower bounds reached chance and the other two included zero, so it was treated as suggestive rather than confirmed. The fine sweep uses split/key/set seeds `90881/90887/90901`, model seeds `467/479/487`, and shared dimensions `0/16/24/32/40/48/56/64` of 128 (`0/12.5/18.75/25/31.25/37.5/43.75/50%`). The descriptive goal is to locate the transition on an independent split; all points and non-monotonicities will be reported.

## FEI third-dataset preregistration (2026-09-12)

Fixed before inspection. FEI face database (Centro Universitario da FEI; research-use, no redistribution), the four official `originalimages_part1-4.zip` archives downloaded from the official page on 2026-09-12 and hashed locally (SHA-256 recorded outside Git in `%USERPROFILE%\ResearchData\FEI\SHA256SUMS.txt`). 2,800 images, 200 identities, 14 images each: 11 profile-rotation steps (poses 01-11), two frontal expressions (12-13), and one low-illumination image (14). Protocol: all 200 identities, 12 of the 14 images per identity chosen by seed 20260912, identity-disjoint 120/40/40 split (1,440/480/480 images). Extraction with the same hash-pinned YuNet and ArcFace models: 2,378/2,400 embeddings; all 22 failures are the low-illumination pose 14, and every identity retains at least 11 valid embeddings, so the held-out-gallery plus ten-exposure construction is preserved. Chance top-1 is `1/40 = 2.50%`.

Run: BioHash 128-bit, randomized pools 1/2/3/4/5/7/10 plus fresh keys, key seed 90697, set seed 90703, model seeds 367/377/387, exposures 1 and 10, mean-pool MLP, unchanged training and per-pool criteria. Purpose: test the fresh-key null and reuse amplification on a controlled high-resolution pose-sweep dataset whose population (institutional volunteers) is unlikely to overlap the ArcFace training set. Expected: fresh keys at chance; leakage under recurring pools decaying with pool size; pass/fail per pool reported as designed. FEI is a single-session dataset and therefore tests pose variation, not session variation.

## SCface added-dataset preregistration (2026-09-18)

Frozen in commit `69a93e4` before result inspection. Authorized SCface data supplied by Sani: all 130 identities, one visible frontal mugshot as held-out gallery and 21 visible surveillance images from seven cameras at three distances as exposure candidates. Infrared and pose mugshots are excluded. Identity-disjoint 78/26/26 split, seed 20260918, expected chance `1/26 = 3.846%`. Advance only if every identity retains the gallery plus at least ten successfully extracted exposures and the unprotected mugshot-to-surveillance baseline is materially above chance.

Run BioHash 128-bit with randomized pools 1/2/3/4/5/7/10 plus fresh keys, key seed 91897, set seed 91903, model seeds 457/467/477, exposures 1 and 10, and mean-pool MLP. Separately run one-seed IoM-GRP/PolyProtect engineering pilots for pools 1/4/8 and fresh keys under `configs/attacks/scface_scheme_extension_pilot.yaml`. The BioHash study is an added-dataset replication; the scheme pilots are not confirmation or a controlled ranking.
