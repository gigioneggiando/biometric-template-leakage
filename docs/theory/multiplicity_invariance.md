...
Corollary 1 (conditional attack bound, corrected 2026-09-12). Suppose the fresh projection keys and postprocessing randomness are jointly independent of $(Y,x_{1:n},Z)$, and their conditional laws given $Z$ retain the theorem's assumptions. The same argument conditional on $Z$ gives $T_{1:n} \perp Y \mid Z$. Observing templates cannot improve the Bayes-optimal success probability beyond that available from $Z$ alone, namely $\mathbb{E}[\max_y P(Y=y\mid Z)]$. If the identity prior remains uniform over the $N$ gallery identities conditional on $Z$, this probability is $1/N$. This is an expected-accuracy bound, not an assertion that a finite empirical accuracy equals chance.

Independence of $Z$ and $Y$ alone, as stated in the earlier draft, is insufficient. Counterexample: let $Y$ and $K$ be independent uniform signs, let $T=KY$, and reveal $Z=K$. Here the one-dimensional projection law is rotation-invariant and both $T$ and $Z$ separately are independent of $Y$, yet $(T,Z)$ recovers $Y$ exactly. A key-bearing or key-correlated gallery/training artifact cannot be treated as harmless merely because its identity labels are disjoint. This correction is a local audit finding; independent human proof review remains open.
...
Update (2026-09-04 experiment; interpretation corrected 2026-09-12): an opt-in `haar_sign_corrected` flag was added and tested (`configs/attacks/mobio_haar_corrected_key_pool.yaml`). Fresh-key ten-record top-1 was `2.64%`, pool 1 `74.58%`, and pool 5 `48.06%`. These are qualitatively similar to the default results, not an equivalence test. Sign correction matches the ideal sampling construction; finite PRNG support and floating-point precision prevent an unqualified claim of exact Haar sampling in the implementation.

Extension scope (2026-09-12): ideal IoM-GRP Gaussian projections followed by argmax satisfy the theorem's fixed-norm projection/postprocessing structure. PolyProtect's ordered polynomial windows do not. Fresh-key PolyProtect leakage would therefore not refute this theorem. Positive scaling leaves zero-threshold BioHash and IoM argmax unchanged; the norm bound is an upper bound and does not assert that either scheme leaks norms. PolyProtect's original paper already studies record multiplicity with disclosed parameters (Section 4.3); hidden-parameter experiments must be distinguished from that work.
