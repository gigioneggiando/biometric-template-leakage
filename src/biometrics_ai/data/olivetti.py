...
class OlivettiProtocolConfig:
    identities: int = 40
    samples_per_identity: int = 10
    seed: int = 20260826


def build_olivetti_protocol(image_root: str | Path, config: OlivettiProtocolConfig) -> list[dict[str, str | int]]:
    image_root = Path(image_root)
    if not image_root.is_dir():
        raise FileNotFoundError(f"Olivetti image root not found: {image_root}")
    ...
    for identity_directory in sorted(path for path in image_root.iterdir() if path.is_dir()):
        images = sorted(identity_directory.glob("*.png"))
    ...
    for protocol_index, eligible_index in enumerate(selected_indices):
        ...
        identity_id = f"olivetti_{protocol_index:04d}"
