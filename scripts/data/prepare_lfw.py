"""Create a deterministic, identity-disjoint LFW engineering protocol."""
...
def main() -> None:
    ...
    parser.add_argument("--image-root", type=Path, default=Path("data/raw/lfw/lfw_home/lfw_funneled"))
    parser.add_argument("--output", type=Path, default=Path("data/interim/lfw_month1_protocol.csv"))
    parser.add_argument("--identities", type=int, default=60)
    parser.add_argument("--samples-per-identity", type=int, default=6)
    parser.add_argument("--seed", type=int, default=20260826)
    args = parser.parse_args()

    config = LfwProtocolConfig(args.identities, args.samples_per_identity, args.seed)
    rows = build_lfw_protocol(args.image_root, config)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    with args.output.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["sample_id", "identity_id", "source_image", "split", "sample_index"])
        writer.writeheader()
