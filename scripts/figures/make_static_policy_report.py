...
def build_report(destination: Path) -> None:
    ...
    with plt.rc_context({"font.family": "DejaVu Sans", "pdf.fonttype": 42}), PdfPages(destination) as output:
        ...
        paragraph(figure, .87, "Contribution and scope",
                  ...
                  "analyzer, not a general source-code analyzer or a new biometric transform. Sani suggested exploring "
                  "an algorithmic contribution; Manish proposed this direction. Sani has not reviewed this specific "
                  "implementation or its conclusions, and research priority is not established.")
