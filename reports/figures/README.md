...
Latest finalized-results package: **22 figures, 19 report pages and 15 slides**. Added [extended exposures](fig_extended_exposures.pdf), [raw learned attacks](fig_raw_learned.pdf), and [local stricter selection](fig_stricter_selection.pdf). The architecture and four-dataset coverage diagram now include these studies. See the [simple page README](../Sept_Dataset_Update_README.md).

The exposure plot uses the extended study's seed/identity intervals and separates both assignments. The raw plot uses seed **SD**, not confidence intervals, and shows no unmatched unit-input points. The stricter plot shows paired native changes with identity intervals and family-2 corrected tests. Source CSVs are in the corresponding [extended](../../experiments/scheme_followup_2026-09-19_full/README.md), [raw](../../experiments/raw_input_attacker_2026-09-19/README.md), and [stricter](../../experiments/polyprotect_stricter_audit_2026-09-19/README.md) study directories. Earlier figure descriptions below retain their historical scope; the current SCface evidence is no longer pilot-only.

**Updated: 2026-09-19**, with the separately frozen [independent-pool study](../../experiments/pool_replication_2026-09-19/README.md): 144 fits plus 72 prediction-mean evaluations. Earlier integration `4352eeb`, [fixed-pool follow-up](../../experiments/scheme_followup_2026-09-18/README.md), SCface freeze `69a93e4`, 73 conditions / 12 studies and one-seed pilots remain separate.
...
| `fig_dataset_coverage` | Four datasets by nine evidence layers, including completed SCface extension | Study protocols and manifests; coverage, not performance |
| `fig_pool_replication` | Three-pool amplification and input-mean versus prediction-mean baseline; crossed pointwise intervals | `experiments/pool_replication_2026-09-19/pool_contrasts.csv`, `crossed_contrasts.csv` |
...
**Independent pools and baseline.** Three independently seeded pool realizations vary both transforms and record-to-slot assignments, with fixed exposure sets. Colored points average three model seeds per pool; black diamonds/bars show the aggregate and crossed pool/seed/identity pointwise 95% intervals (4,000 draws). IoM gains persist; PolyProtect has three reversals and wide pool variation. All eight direct input-mean minus prediction-mean intervals include zero. The baseline reuses the single-record model on identical ten-record sets: inference is matched, training objectives are not. Three pools do not establish universal robustness or corrected significance. The coverage diagram shows study presence, not effect size; grey is missing, not zero.

The [complete figure appendix](../slides/figure_appendix.pdf) collects all 22 current vector figures. The 73-condition overview covers earlier studies only; pilot and follow-up plots remain separate.

The [19-page September update](../Sept_Dataset_Update.pdf) is generated by the same command as the deck, with a [simple README](../Sept_Dataset_Update_README.md) and [detailed page guide](../Sept_Dataset_Update_guide.md). It preserves vector diagrams and searchable labels; heatmap cells remain intentional raster layers. Retain current PNG/PDF pairs, editable deck and appendix; only stale local audit previews are disposable.

[15-slide PDF](../slides/research_review.pdf) and [editable PowerPoint](../slides/research_review.pptx) are generated together. Slide text/tables are editable; figures are PNGs with Python sources and separate vector PDFs. No biometric photographs are included. Native PowerPoint rendering needs a check on the presenting machine; matching PDFs have text-region and nonblank-page tests.
