...
Continue requesting a corrected `benchmark_cb` source; keep the reproduction blocked until then. Densely map the 5-to-10 key threshold, repeat it over protocol seeds and MLP-Hash, and add non-normalized, same-image/different-key, and shuffled-record controls. A high-tier claim still requires a formal proof reviewed for assumptions, at least one source-exact published transform, additional datasets, and statistical equivalence rather than failure-to-reject significance tests.
