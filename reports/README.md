...
The local per-seed inventory still has 633 rows from 25 artifacts and excludes SCface because its detailed run artifacts are not present on this host. Current SCface aggregate CSVs are tracked in their study directories. Documentation date is not a new experiment date; acquisition records are linked from the [data README](../data/README.md).
